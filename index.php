...
// Function to send USSD response
function sendResponse($response) {
    header('Content-type: text/plain');
    echo $response;
}

// Function to handle the "next" option
function handleNextOption($text, $currentValue) {
    // Logic to determine the next set of options based on the current value
    // You can customize this based on your application's requirements
    // For demonstration, let's just increment the current value by 1
    $nextValue = $currentValue + 1;

    // Construct the next set of options with the updated value
    $response = "CON Next set of options for value $nextValue\n";
    $response .= "1) Next Option 1\n";
    $response .= "2) Next Option 2\n";
    // Add more options as needed...
    $response .= "9) Go to next\n"; // Option to go to next set of options
...
if ($text == "") {
    ...
    // Business logic for third level response - part 1
    $response = "CON Ibiribwa bitera imbaraga(ibinyamafufu)\n\n";
    $response .= "-Ibirayi\n";
    // Add more options here...
} else if ($text == "2*2*1*next") {
    // Handle next option with current value
    $response = handleNextOption($text, 1); // Assuming the current value is 1
} else if ($text == "2*2*2") { 
    // Business logic for third level response - part 2
    $response = "CON Ibyubaka umubiri(Ibinyameke n'ibikomoka ku matungo)\n\n";
    // Add options...
} else if ($text == "2*2*2*next") {
    // Handle next option with current value
    $response = handleNextOption($text, 1); // Assuming the current value is 1
} else if ($text == "2*2*3") { 
    // Business logic for third level response - part 3
    $response = "CON Ibirinda indwara(imboga n'imbuto)\n\n";
    // Add options...
} else if ($text == "2*2*3*next") {
    // Handle next option with current value
    $response = handleNextOption($text, 1); // Assuming the current value is 1
}
